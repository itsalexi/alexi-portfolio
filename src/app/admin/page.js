...
export default function AdminPage() {
  const [contentType, setContentType] = useState("projects"); // "projects" or "talks"
  ...
  const availableTech = [
    "nextjs", "react", "vue", "tailwind", "typescript", "javascript",
    "nodejs", "python", "go", "rust", "express", "fastapi",
    "postgresql", "mongodb", "redis", "mysql", "docker", "kubernetes",
    "git", "github", "aws", "vercel", "graphql", "firebase", "supabase"
  ];
  ...
  const toggleTech = (tech) => {
    ...
    if (currentTechs.includes(tech)) {
      ...
      // Add
      setFrontmatter({...frontmatter, techStack: [...currentTechs, tech].join(", ")});
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid md:grid-cols-4 gap-6">
          ...
          <div className="md:col-span-1">
            ...
            >
              + New {contentType === "projects" ? "Project" : "Talk"}
            </button>
            ...
            {selectedProject ? (
                ...
                <div className="bg-white/5 rounded-lg p-6 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFrontmatter({...frontmatter, tagline: e.target.value})}
                      className="w-full px-4 py-2 bg-white/10 rounded-lg border border-white/20 focus:border-blue-500 focus:outline-none"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3 min-h-[40px] p-2 bg-white/5 rounded-lg border border-white/10">
                      {getSelectedTechs().length > 0 ? (
                        ...
                        <span className="text-white/40 text-sm">Click tags below to add</span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto">
                      ...
                      {getFilteredTech().length === 0 && (
                        <span className="text-white/40 text-sm">No matches found</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFrontmatter({...frontmatter, liveUrl: e.target.value})}
                        className="w-full px-4 py-2 bg-white/10 rounded-lg border border-white/20 focus:border-blue-500 focus:outline-none"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFrontmatter({...frontmatter, githubUrl: e.target.value})}
                        className="w-full px-4 py-2 bg-white/10 rounded-lg border border-white/20 focus:border-blue-500 focus:outline-none"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block text-sm text-white/60 mb-2">Featured Image (for project card)</label>
                    <div className="flex items-center gap-4">
                      {frontmatter.image && (
                        <img src={frontmatter.image} alt="Featured" className="w-32 h-20 object-cover rounded-lg border border-white/20" />
                      )}
                  ...
                  <div>
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-white/60">Show this project on the homepage (max 2)</p>
                      </div>
                    </label>
                  </div>
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white/5 rounded-lg p-6 space-y-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm text-white/60 mb-2">Event</label>
                      <input
                        type="text"
                        value={talkData.event}
                        onChange={(e) => setTalkData({...talkData, event: e.target.value})}
                        className="w-full px-4 py-2 bg-white/10 rounded-lg border border-white/20 focus:border-blue-500 focus:outline-none"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setTalkData({...talkData, shortDescription: e.target.value})}
                      rows={2}
                      className="w-full px-4 py-2 bg-white/10 rounded-lg border border-white/20 focus:border-blue-500 focus:outline-none"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm text-white/60 mb-2">Topics (comma separated)</label>
                    <input
                      type="text"
                      value={talkData.topics}
                      onChange={(e) => setTalkData({...talkData, topics: e.target.value})}
                  ...
                  <div>
                    <label className="block text-sm text-white/60 mb-2">Link URL</label>
                    <input
                      type="text"
                      value={talkData.link}
                      onChange={(e) => setTalkData({...talkData, link: e.target.value})}
                      placeholder="https://..."
                      className="w-full px-4 py-2 bg-white/10 rounded-lg border border-white/20 focus:border-blue-500 focus:outline-none"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm text-white/60 mb-2">Carousel Images</label>
                    <div className="space-y-3">
                          ...
                          {talkData.images.map((img, idx) => (
                            <div key={idx} className="relative group">
                              ...
                              <button
                                ...
                                onClick={() => setTalkData({
                                  ...talkData,
                                  images: talkData.images.filter((_, i) => i !== idx)
                                })}
                      ...
                      <label className="flex items-center gap-2 px-4 py-2 bg-purple-500 hover:bg-purple-600 rounded-lg cursor-pointer transition-colors w-fit">
                        <Upload className="w-4 h-4" />
                        <span className="text-sm">{uploading ? "Uploading..." : "Upload Carousel Images"}</span>
                        <input
                          type="file"
                          accept="image/*"
                          multiple
                          onChange={async (e) => {
                            ...
                            if (uploadedUrls.length > 0) {
                              setTalkData({
                                ...talkData,
                                images: [...(talkData.images || []), ...uploadedUrls]
                              });
                            }
                      ...
                      <details className="text-sm">
                        <summary className="cursor-pointer text-white/60 hover:text-white/80">Or enter URLs manually</summary>
                        <textarea
                          value={Array.isArray(talkData.images) ? talkData.images.join("\n") : ""}
                          onChange={(e) => setTalkData({
                            ...talkData, 
                ...
                <div className="bg-white/5 rounded-lg p-6">
                  <h2 className="text-xl font-bold mb-4">Full Description (Markdown)</h2>
                  <div data-color-mode="dark">
                    <MDEditor
                      value={content}
                      onChange={(val) => setContent(val || "")}
              ...
              <div className="bg-white/5 rounded-lg p-12 text-center text-white/60">
                Select a {contentType === "projects" ? "project" : "talk"} or create a new one to start editing
              </div>
            )}
          </div>
        </div>
