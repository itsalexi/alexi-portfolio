        ...
        >
          <ReactMarkdown
            components={{
              ...
              code: ({node, ...props}) => (
                <code className="text-blue-400 bg-blue-500/10 px-2 py-1 rounded text-sm" {...props} />
              ),
              img: ({node, ...props}) => (
                <img {...props} className="w-full my-8 rounded-xl border border-white/10 shadow-2xl" loading="lazy" alt={props.alt || "Project image"} />
              )
            }}
          >
            {project.content}
